...
class CassieEnv_stand:
    ...
    def reset(self):
        ...
        push = 50*random.choice([-1, 1]) #random.randint(-10, 10) / 10.0
    ...
    def compute_reward(self):
        ...
        #height_diff = np.abs(qpos[2] - .8)
        pos_diff = np.linalg.norm(qpos[7:] - stand_pos[7:])
        pel_vel = np.linalg.norm(qvel[0:3])
        reward = .5*np.exp(-pos_diff) + .5*np.exp(-pel_vel)
