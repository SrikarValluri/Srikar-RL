...
import numpy as np
from rl.utils.render import renderpolicy
...
def get_rewards(env, policy, num_trj, deterministic=True):
    r_ep = 0
    R = []
    done = False
...
def plot_reward_dist(env, policy, num_trj, deterministic=True):
    r_ep = 0
    R = []
    done = False
...
def plot_action_dist(env, policy, num_trj, deterministic=True):
    n = 0
    A = []
    done = False
...
parser.add_argument("--vlen", type=int, default=150,
...
if __name__ == "__main__":
    ...
    if args.new:
        ...
        if args.visualize:
            visualize(env, policy, args.vlen, deterministic=True)
        ...
        #     env.render()
        #     input()
        renderpolicy(env, policy, deterministic=False, speedup=1, dt=0.05)
        # if args.visualize:
